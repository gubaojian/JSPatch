...
/***
 * for demo use only, product use
 * see https://github.com/gubaojian/DuktapeJava
 * */
public class AssetScript {
	...
	public static String transformScript(String plainScript){
		try {
			String script =  JSTransformer.parse(new StringReader(plainScript));
			Log.e("ScriptEngine", script);
			return script;
		} catch (Exception e) {
			throw new RuntimeException(e);
